...
const TechStack = () => {
  ...
  const row1 = ["C++", "C#", "Java", "TypeScript", "JavaScript", "React", "Nextjs", "WPF"];
  // Row 2: Backend & Database (서버 및 데이터베이스)
  const row2 = ["NestJS", "Nodejs", "Express", "MySQL", "SQLite", "Redis", "TypeORM"];
  // Row 3: System & Tools (시스템 라이브러리 및 인프라)
  const row3 = ["OpenCV", "Docker", "Nginx", "CMake", "Win32-API", "Github-Actions", "Firebase"];
  ...
  const ScrollingRow = ({ items, direction }: { items: string[], direction: 'left' | 'right' }) => (
    <div className="flex overflow-hidden py-[8px] select-none">
      <div className={`flex gap-[24px] whitespace-nowrap ${direction === 'left' ? 'animate-scroll-left' : 'animate-scroll-right'}`}>
        {[...items, ...items].map((item, idx) => (
          ...
          >
            ...
            <span className="text-[18px] font-bold text-zinc-700 tracking-tight">{item}</span>
          </div>
        ))}
      </div>
    </div>
